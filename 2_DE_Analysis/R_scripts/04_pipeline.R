...
# edgeR 결과 테이블을 DESeq2 스타일 컬럼 구조로 변환하는 함수
...
# DEG 결과를 표준화하고 엑셀 저장 및 volcano용 컬럼을 추가하는 함수
...
# 개별 comparison 결과에 대해 downstream 분석 전체를 실행하는 함수
...
# 입력 준비부터 edgeR DEG 계산까지 전체 파이프라인을 실행하는 함수
run_deg_pipeline <- function(project_settings, root_path) {
  # 프로젝트 설정과 입력 파일을 읽어 분석용 객체를 준비하는 내부 함수
  ...
  # TPM 테이블과 log2(TPM+1) 테이블을 저장하는 내부 함수
  ...
  # edgeR용 DGEList와 design matrix를 구성하는 내부 함수
  ...
  # TMM normalized count와 log-TMM 테이블을 저장하는 내부 함수
  ...
  # 지정한 비교군 조합별로 edgeR contrast DEG 결과를 계산하는 내부 함수
